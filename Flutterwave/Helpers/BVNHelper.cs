...
namespace Flutterwave.Helpers
{
    public class BVNHelper
    {
        ...
        public BVNHelper()
        {
            resources.Add(Environments.Staging, new BVNOperation
            {
            ...
            resources.Add(Environments.Production, new BVNOperation
            {
        ...
        public async Task<string> Verify(string bvn, Driver driver, string otpOption = Verification.SMS)
        {
            ...
            var response = await new Request(resource).MakeRequest(data: verifyBVN, method: Verbs.POST);
            return response;
        }
        public async Task<string> Validate(string bvn, string otp, string transactionReference, Driver driver)
        {
            ...
            var response = await new Request(resource).MakeRequest(data: validateBVN, method: Verbs.POST);
            return response;
        }
        public async Task<string> ResendOtp(string transactionReference, Driver driver, string otpOption = Verification.SMS)
        {
            ...
            var response = await new Request(resource).MakeRequest(data: resendOTPBVN, method: Verbs.POST);
            return response;
        }
    }
}
